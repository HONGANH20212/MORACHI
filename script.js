...
// FORMAT GIÁ TIỀN: Định dạng đúng chuẩn 174.000đ (không có dấu cách)
function formatPrice(value) {
    const number = parsePrice(value);
    return number.toLocaleString("vi-VN") + "đ";
}
...
// --- Hàm xử lý Click thả tim yêu thích ---
...
function renderProducts(products) {
    ...
    productList.innerHTML = products.map((product) => {
        ...
        // 1. CHỈ HIỂN THỊ NHÃN DO ADMIN NHẬP (Tuyệt đối không hiện nhãn Hết hàng/Order ở đây)
        let discountBadgeHTML = "";
        const rawDiscount = product.discount || "";
        const lowerDiscount = rawDiscount.toLowerCase();
        ...
        if (rawDiscount) {
            // Nếu admin nhập chữ "bán chạy" hoặc "hot" thì cho nền cam gradient + icon Lửa
            if (lowerDiscount.includes("bán chạy") || lowerDiscount.includes("hot")) {
                discountBadgeHTML = `<span class="badge-hot"><i class="fas fa-fire"></i> ${escapeHtml(rawDiscount)}</span>`;
            } else {
                // Các trường hợp khác (như -20%, Mới) thì giữ nền bình thường
                discountBadgeHTML = `<span class="discount-badge">${escapeHtml(rawDiscount)}</span>`;
            }
        }

        // CẤU TRÚC HTML THẺ SẢN PHẨM HOÀN TOÀN MỚI THEO THIẾT KẾ
        return `
            <div class="product-card" onclick="window.location.href='product-detail.html?id=${id}'">
                ...
                <!-- ICON TRÁI TIM GÓC TRÊN CÙNG -->
                <div class="wishlist-icon" onclick="event.stopPropagation(); window.toggleWishlist(this)" title="Thêm vào yêu thích">
                    <i class="fa-regular fa-heart"></i>
                </div>
                
                <img class="product-img" src="${thumbnail}" alt="${title}" onerror="this.src='images/icon-logo.png'">
                ...
                <div class="product-info">
                    ...
                    <!-- NÚT MUA NGAY TO BẢN CHUẨN DESIGN -->
...
async function loadProducts() {
    ...
    if (isCacheValid) {
        try {
            ...
            state.allProducts = Array.isArray(products) ? products : [];
            renderBrandFilters(state.allProducts);
            applyClientFilters(); // Render ngay lập tức
            return; 
        } catch(e) {
            console.error("Lỗi đọc cache:", e);
        }
